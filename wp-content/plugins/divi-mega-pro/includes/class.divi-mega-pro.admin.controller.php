<?php
	...
	class DiviMegaPro_Admin_Controller {
		...
		public static function get_posts( $args = null ) {
			...
			add_filter( 'posts_where', array( 'DiviMegaPro_Admin_Controller', 'divimegapro_title_filter'), 10, 2 );
			$query = new WP_Query( $args );
			remove_filter( 'posts_where', array( 'DiviMegaPro_Admin_Controller', 'divimegapro_title_filter'), 10, 2 );
		...
		public static function divimegapro_title_filter( $where, $wp_query )
		{
			global $wpdb;
			
			if ( $search_term = $wp_query->get( 'divimegapro_by_title_like' ) ) {
		...
		public static function divimegapros_displaylocations_callback( $post ) {
			...
			$archive = get_post_meta( $post->ID, 'dmp_displaylocations_archive' );
			...
			$author = get_post_meta( $post->ID, 'dmp_displaylocations_author' );
			...
			<div class="divilife_meta_box">
				<div class="at_pages">
					<select name="dmp_css_selector_at_pages" class="at_pages chosen dmp-filter-by-pages" data-dropdownshowhideblock="1">
						<option value="all"<?php if ( $at_pages == 'all' ) { ?> selected="selected"<?php } ?> data-showhideblock=".do-list-exceptionpages-container"><?php esc_html_e( 'All posts and pages', 'DiviMegaPro' ); ?></option>
						<option value="posts"<?php if ( $at_pages == 'posts' ) { ?> selected="selected"<?php } ?> data-showhideblock=""><?php esc_html_e( 'All posts', 'DiviMegaPro' ); ?></option>
						<option value="pages"<?php if ( $at_pages == 'pages' ) { ?> selected="selected"<?php } ?> data-showhideblock=""><?php esc_html_e( 'All pages', 'DiviMegaPros' ); ?></option>
				...
				<div class="divilife_meta_box">
					<p>
						<input name="dmp_displaylocations_archive" type="checkbox" id="dmp_displaylocations_archive" value="1" <?php checked( $archive[0], 1 ); ?> /> <?php esc_html_e( 'Archive', 'DiviMegaPro' ); ?>
					</p>
				</div>
				<div class="divilife_meta_box">
					<p>
						<input name="dmp_displaylocations_author" type="checkbox" id="dmp_displaylocations_author" value="1" <?php checked( $author[0], 1 ); ?> /> <?php esc_html_e( 'Author', 'DiviMegaPro' ); ?>
					</p>
				</div>
				<div class="divilife_meta_box">
					<p>
		...
		public static function divimegapros_displaysettings_callback( $post ) {
			...
			$dmp_cssposition = get_post_meta( $post->ID, 'dmp_cssposition', true );
			...
			<?php 
			if ( !isset( $dmp_cssposition ) || $dmp_cssposition == '' ) {
				
				$dmp_cssposition = 'absolute';
			}
			
			$dmp_ats = array(
				'absolute' => esc_html__( 'Absolute', 'DiviMegaPro' ),
				'fixed' => esc_html__( 'Fixed', 'DiviMegaPro' ),
			);
			?>
			<div class="divilife_meta_box">
				<p class="dmp_cssposition et_pb_single_title">
					<label for="dmp_cssposition"><?php esc_html_e( 'CSS Position', 'DiviMegaPro' ); ?>:</label>
					<select id="dmp_cssposition" name="dmp_cssposition" class="dmp_cssposition chosen">
					<?php
					foreach ( $dmp_ats as $at_value => $at_name ) {
						printf( '<option value="%2$s"%3$s>%1$s</option>',
							esc_html( $at_name ),
							esc_attr( $at_value ),
							selected( $at_value, $dmp_cssposition, false )
						);
					} ?>
					</select>
				</p>
		...
		public static function save_post( $post_id, $post ) {
			...
			/* By Archive */
			if ( isset( $_POST['dmp_displaylocations_archive'] ) ) {
				
				$dmp_displaylocations_archive = 1;
				
			} else {
				
				$dmp_displaylocations_archive = 0;
			}
			update_post_meta( $post_id, 'dmp_displaylocations_archive', $dmp_displaylocations_archive );
			
			/* By Author */
			if ( isset( $_POST['dmp_displaylocations_author'] ) ) {
				
				$dmp_displaylocations_author = 1;
				
			} else {
				
				$dmp_displaylocations_author = 0;
			}
			update_post_meta( $post_id, 'dmp_displaylocations_author', $dmp_displaylocations_author );
			...
			if ( isset( $_POST['dmp_cssposition'] ) ) {
				
				$post_value = sanitize_text_field( wp_unslash( $_POST['dmp_cssposition'] ) );
				update_post_meta( $post_id, 'dmp_cssposition', $post_value );
			}
			...
			DiviMegaPro_Admin::super_clear_cache();
			
			return $post_id;
		}
		
		public static function save_post_not_divimegapro( $post_id, $post ) {
