jQuery(function($){
	...
	$(".remove-sched-btn").do_confirm({confirm_message : wlm.translate( 'Delete this schedule?' ), yes_button : 'Delete', placement: 'right'}).on("yes.do_confirm", remove_schedule );
	$('.update-sched-btn').click(content_action_changed);

	$('.enable-content-control-switch').each(function() {
		enable_modules = enable_modules + parseInt($(this).attr("option_value"));
    });
	//for toggle boxes
	$('.enable-content-control-switch').on('click', function() {
...
var content_action_changed = function(){
	...
	switch ( operation ) {
	    case "scheduler":
	    	...
	    	if ( disable_dropdown ) {
	    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Edit Content Schedule (Drip)' ));
	    		$("#" +modal_id).find(".save-button span").html("Update Content Schedule");
	    		$(".confirm-save-button").click( set_schedule );
	    	} else {
	    		if ( remove ) {
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Remove Content Schedule (Drip)' ));
		    		...
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Set Content Schedule (Drip)' ));
					...
					if ( confirm_protection ) {
						$(".confirm-save-button").do_confirm({confirm_message : wlm.translate( 'Content must be protected in order for Content Scheduling to function properly.<br />Protect it now?' ), yes_button : 'Yes',no_button : 'Cancel', placement: 'bottom'}).on("yes.do_confirm", set_schedule );
					} else {
						$(".confirm-save-button").click( set_schedule );
					}
	    ...
	    case "archiver":
	        ...
	        if ( disable_dropdown ) {
	        	$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Edit Content Archive Date' ));
	        	$("#" +modal_id).find(".confirm-save-button span").html("Update Archive Date");
	        	$(".confirm-save-button").click( set_schedule );
	        } else {
	    		if ( remove ) {
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Remove Content Archive Date' ));
		    		...
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Set Content Archive Date' ));
					...
					if ( confirm_protection ) {
						$(".confirm-save-button").do_confirm({confirm_message : wlm.translate( 'Content must be protected in order for Content Archiving to function properly.<br />Protect it now?' ), yes_button : 'Yes',no_button : 'Cancel', placement: 'bottom'}).on("yes.do_confirm", set_schedule );
					} else {
						$(".confirm-save-button").click( set_schedule );
					}
	    ...
	    case "manager":
	    	...
	    	if ( disable_dropdown ) {
	    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Edit Content Manager Date' ));
	    		$("#" +modal_id).find(".confirm-save-button span").html("Update Manager Date");
	    	} else {
	    		if ( remove ) {
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Remove Content Manager Date' ));
		    		$("#" +modal_id).find(".confirm-save-button span").html("Remove Content Manager Date");

					$("#" +modal_id).find(".confirm-save-button").removeClass("-primary").addClass("-danger");
					$("#" +modal_id).find(".confirm-save-button .wlm-icons").html("remove_circle_outline");
	    		} else {
		    		$("#" +modal_id).find(".modal-title").html(wlm.translate( 'Add Content Manager Date' ));
	...
	if ( disable_dropdown && operation == "manager" ) action_changed(action);
...
var remove_schedule = function( e ) {
	...
	if ( operation == "remove_scheduler" ) wlm_action = "set_content_schedule";
	if ( operation == "remove_archiver" ) wlm_action = "set_content_archive";
	if ( operation == "remove_manager" ) wlm_action = "set_content_manager";
	    	...
	    	if ( $result.success && $result.data ) {
	    		...
	    		$this_button.closest("tr").fadeOut( 500, function(){
		    		...
		    		if ( $holder.find('tr').length <= 0 ) {
		    			...
		    			var lvls = [];
		    			$holder.find('tr .wlm-level-name').each( function() {
							lvls.push($(this).html());
						});
