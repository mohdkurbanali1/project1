jQuery(function($){
	...
	$(".wlm-payperpost-users").select2({
		...
		minimumInputLength: 2, placeholder: wlm.translate( 'Search for members' ),theme:"bootstrap",
	});
});
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
		        ...
		        $.each( $result.data, function ( uid, data ) {
					...
					$markup += '</tr>';
					var $el = $($markup);
					$el.find(".remove-ppp-user-btn").do_confirm({confirm_message : wlm.translate( 'Remove User from Pay Per Post?' ), yes_button : wlm.translate( 'Remove' ), placement: 'right'}).on("yes.do_confirm", remove_payperpost_user );
		    		$holder.append($el);
		        })
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
	    		if ( $result.content.post_title ) {
	    			title = "<span title='" +$result.content.post_title +"' style='display:inline-block;margin:0px;padding:0px;width:400px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;'>Edit: ";
	    		...
	    		if ( !select.data('select2') ) select.select2({theme:"bootstrap", placeholder: wlm.translate( 'Select Membership Levels' )});
				...
				$("#" +modal_id).find(".remove-ppp-user-btn").do_confirm({confirm_message : wlm.translate( 'Remove User from Pay Per Post?' ), yes_button : wlm.translate( 'Remove' ), placement: 'right'}).on("yes.do_confirm", remove_payperpost_user );
				// $("#" +modal_id).find(".remove-ppp-user-btn").click( remove_payperpost_user );

				$("#" +modal_id).find(".add-contentlvl-btn").click( add_contentlvl );
				$("#" +modal_id).find(".remove-contentlvl-btn").do_confirm({confirm_message : wlm.translate( 'Remove Level from Content?' ), yes_button : wlm.translate( 'Remove' ), placement: 'right'}).on("yes.do_confirm", remove_contentlvl );
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
		    	if ( $result.content ) {
					...
					$(".wlm-message-holder").show_message({message:$result.msg, msg_typee:$result.msg_type, icon:$result.msg_type});
...
var add_contentlvl = function() {
	...
	if ( !$lvlid ) { $(".wlm-message-holder").show_message({message:wlm.translate( 'No level selected.' ), type: "danger" }); return; }
	if ( !$content_type ) { $(".wlm-message-holder").show_message({message:wlm.translate( 'Invalid content type.' ), type: "danger" }); return; }
	if ( !$contentids ) { $(".wlm-message-holder").show_message({message:wlm.translate( 'No content selected.' ), type: "danger" }); return; }
	var settings_data = {
		wlm_levels: $lvlid,
		content_type: $content_type,
		contentids: $contentids,
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
	    		...
	    		$.each( $lvlname, function( $i, $n ) {
					...
					$markup += '</tr>';
					var $el = $($markup);
					$el.find(".remove-contentlvl-btn").do_confirm({confirm_message : wlm.translate( 'Remove Level from Content?' ), yes_button : wlm.translate( 'Remove' ), placement: 'right'}).on("yes.do_confirm", remove_contentlvl );
		    		$holder.append($el);
				...
				$this_button.closest(".row").find(".add-wlm-levels").select2({theme:"bootstrap",placeholder: wlm.translate( 'Select Membership Levels' )});
	...
	var x = $this_button.save_settings({
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
				...
				$(".add-wlm-levels").select2({theme:"bootstrap",placeholder: wlm.translate( 'Select Membership Levels' )});
...
var toggle_content_protection = function (e) {
	...
	if ( protection == "Inherited" ) {
		protection = wlm.translate( 'Protected' );
		newicon = "lock";
	} else if( protection == "Protected") {
		protection = wlm.translate( 'Unprotected' );
		newicon = "lock_open";
	} else {
		protection = wlm.translate( 'Inherited' );
		newicon = "inherit";
	}
...
var level_action_changed = function() {
    ...
    if ( ids.length <= 0 ) {
    	$(".wlm-message-holder").show_message({message:wlm.translate( 'No content selected.' ), type: "danger" });
    	return;
    }
	var modal_id = "";
	switch ( operation ) {
	    ...
	    case "addpppusers":
			...
			$("#" +modal_id).find(".save-button .wlm-icons").next().html(" Add Member");
			break;
	    case "removepppusers":
	        ppp_user_modal.open();
			modal_id = ppp_user_modal.data.id;
			...
			$("#" +modal_id).find(".save-button .wlm-icons").next().html(" Remove Member");
			break;
	    default:
...
$.getScript(WLM3VARS.pluginurl + '/ui/js/admin_js/content_protection/posts/payperpost.js?build=8261');