	...
	$(".folder-list").save_settings({
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
	    		...
	    		console.log(wlm.translate( 'Unable to retrieve files' ));
	    	}
	    }
	});
}
   ...
   $(this).parent().parent().parent().parent().parent().save_settings({
	    on_init: function( $me, $data) {
	    	if ( $data.folder_protection_autoconfig == "1" ) {
	    		$( '.wlm-message-holder' ).show_message({ message : wlm.translate( 'Enabling auto-configure...' ) });
	    	} else {
	    		$( '.wlm-message-holder' ).show_message({ message : wlm.translate( 'Disabling auto-configure...' )});
	    	}
	    },
        on_success: function( $me, $result) {
            $me = $me.find(".notification-switch");
	    	if ( $result.data.folder_protection_autoconfig == "1" ) {
	    		$( '.wlm-message-holder' ).show_message({ message : wlm.translate( 'Auto-Configure enabled' ) });
	    	} else {
	    		$( '.wlm-message-holder' ).show_message({ message : wlm.translate( 'Auto-Configure disabled' )});
	    	}
        },
...
var show_files_clicked = function() {
	...
	$("#" +modal_id).find(".modal-title").html( wlm.translate( 'Files inside the folder <strong>$$1</strong>', [$this_button.attr('data-path')] ) );
