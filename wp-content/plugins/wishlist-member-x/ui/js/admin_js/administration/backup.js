...
jQuery(function($){
	...
	$(".delete-backup-btn").do_confirm({confirm_message : wlm.translate( 'Delete Backup?' ), yes_button : wlm.translate( 'Delete' ), placement: 'right'}).on("yes.do_confirm", confirm_delete_backup );
	...
	$(".backup-cancel-btn").do_confirm({confirm_message : wlm.translate( 'Do you want to cancel this backup?' ), yes_button : wlm.translate( 'Yes' ), placement: 'right'}).on("yes.do_confirm", confirm_cancel_backup );
});
...
var show_restore_backup_modal = function( e ) {
	...
	var msg =wlm.translate( 'Do you want to restore the backup you made on <strong>$$1</strong>?', [data_date] );
	...
	var x = $body.save_settings({
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.success ) {
				...
				$("#backup-holder").find('.delete-backup-btn').do_confirm({confirm_message : wlm.translate( 'Delete Backup?' ), yes_button : wlm.translate( 'Delete' ), placement: 'right'}).on("yes.do_confirm", confirm_delete_backup );
...
var restore_backup = function() {
	...
	var x = $body.save_settings({
	    on_init: function( $me, $data) {
	    	$this_button.disable_button({disable:true, text:wlm.translate( 'Restoring...' ), icon:"update"});
			$("#" +modal_id).on('hide.bs.modal', function(e) {
				if ( $this_button.prop("disabled") || $this_button.hasClass("-disable") || $this_button.hasClass("-disabled") ) {
					e.preventDefault();
				}
			});
			var msg = wlm.translate( 'Please wait while we restore your settings. It might take some time...' );
	    ...
	    on_fail: function( $me, $data) {
	    	alert(WLM3VARS.request_failed);
	    	$this_button.disable_button({disable:false, text:wlm.translate( 'Yes' ), icon:"restore"});
	    },
	    on_error: function( $me, $error_fields) {
	    	alert(WLM3VARS.request_error);
	    	$this_button.disable_button({disable:false, text:wlm.translate( 'Yes' ), icon:"restore"});
