...
jQuery(function($){
	...
	$(".import-cancel-btn").do_confirm({confirm_message : wlm.translate( 'Do you want to cancel this import?' ), yes_button : wlm.translate( 'Yes' ), placement: 'right'}).on("yes.do_confirm", confirm_cancel_import );
});
	...
	var x = $this_button.save_settings({
	    ...
	    on_success: function( $me, $result) {
	    	if ( $result.data.import_action && $result.data.import_action == "start" ) {
	    		queue_ticker = setInterval( update_queue_counter, queue_interval );
	    		$('.import-status').html("");
	    		$this_button.attr("title",wlm.translate( 'Pause Import' ));
	    		$this_button.attr("action","pause");
	    		$this_button.find(".wlm-icons").html("pause");
	    	} else {
	    		$('.import-status').html(wlm.translate( 'PAUSED' ));
	    		$this_button.attr("title",wlm.translate( 'Start Import' ));
