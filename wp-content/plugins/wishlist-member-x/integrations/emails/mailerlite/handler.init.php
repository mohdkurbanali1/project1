...
add_action(
	...
	function( $old = '', $new = '' ) {
		$ar = ( new \WishListMember\Autoresponder( 'mailerlite' ) );
			...
			foreach ( $ar->settings['lists'] as $level => $listid ) {
				if ( $listid ) {
					...
					if ( ! empty( $ar->settings['unsubscribe'][ $level ] ) ) {
						$ar->settings['list_actions'][ $level ]['removed'] = array( 'remove' => (array) $listid );
					}
				}
			}
...
$class_name = '\WishListMember\Autoresponders\MailerLite';
