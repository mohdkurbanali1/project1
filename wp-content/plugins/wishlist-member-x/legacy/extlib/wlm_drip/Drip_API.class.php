...
class WLM_Drip_Api {
	...
	/**
	 *
	 * This calls RETRIEVE /:account_id/tags to remove the tags. It just returns some status code no content
	 *
	 * @param array $params
	 * @param bool  $status success or failure
	 */
	...
	public function make_request( $url, $params = array(), $req_method = self::GET ) {
		...
		if ( self::POST == $req_method ) { // We want post but no params to supply. Probably we have a nice link structure which includes all the info.
			curl_setopt( $ch, CURLOPT_CUSTOMREQUEST, 'POST' );
		} elseif ( self::DELETE == $req_method ) {
			curl_setopt( $ch, CURLOPT_CUSTOMREQUEST, 'DELETE' );
		} elseif ( self::PUT == $req_method ) {
			curl_setopt( $ch, CURLOPT_CUSTOMREQUEST, 'PUT' );
		}

		if ( ! empty( $params ) ) {
			if ( ( isset( $params['__req'] ) && 'get' == strtolower( $params['__req'] ) )
					|| self::GET == $req_method ) {
				unset( $params['__req'] );
				$url .= '?' . http_build_query( $params );
			} elseif ( self::POST == $req_method || self::DELETE == $req_method ) {
