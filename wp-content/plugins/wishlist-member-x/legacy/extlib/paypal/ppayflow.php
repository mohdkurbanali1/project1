...
class PPayflow {
	...
	public function __construct( $DataArray) {
		...
		$this->APIButtonSource  = 'WishListProducts_SP';
		$this->PathToCertKeyPEM = '/path/to/cert/pem.txt';
		$this->SSL              = isset($_SERVER['SERVER_PORT']) && '443' == $_SERVER['SERVER_PORT'] ? true : false;
		...
		$this->NVPCredentials .= 'Signature' == $this->APIMode ? '&SIGNATURE=' . $this->APISignature : '';
	 ...
	 */
	public function CURLRequest( $Request = '', $APIName = '', $APIOperation = '', $PrintHeaders = false) {
		$curl = curl_init();
	...
	public function APICallSuccessful( $ack) {
		if ('SUCCESS' != strtoupper($ack) && 'SUCCESSWITHWARNING' != strtoupper($ack) && 'PARTIALSUCCESS' != strtoupper($ack)) {
			return false;
		} else {
			return true;
		}
	...
	public function WarningsReturned( $ack) {
		if (false !== strpos(strtoupper($ack), 'WARNING')) {
			return true;
		} else {
			false;
		}
	...
	public function DisplayErrors( $Errors) {
		foreach ($Errors as $ErrorVar => $ErrorVal) {
			...
			foreach ($CurrentError as $CurrentErrorVar => $CurrentErrorVal) {
				if ('L_ERRORCODE' == $CurrentErrorVar) {
					$CurrentVarName = 'Error Code';
				} elseif ('L_SHORTMESSAGE' == $CurrentErrorVar) {
					$CurrentVarName = 'Short Message';
				} elseif ('L_LONGMESSAGE' == $CurrentErrorVar) {
					$CurrentVarName = 'Long Message';
				} elseif ('L_SEVERITYCODE' == $CurrentErrorVar) {
	 ...
	 * @return bool
	 */
	public function Logger( $log_path, $filename, $string_data) {

		if ($this->LogResults) {
			$timestamp = strtotime('now');
			$timestamp = wlm_date('mdY_gi_s_A_', $timestamp);
	 ...
	 * @return	mixed[]	Returns an array structure of the PayPal HTTP response params as well as parsed errors and the raw request/response.
	 */
	public function UpdateAuthorization( $DataArray) {
	...
	public function SetExpressCheckout( $DataArray) {
		...
		foreach ($SECFields as $SECFieldsVar => $SECFieldsVal) {
			if ('SKIPDETAILS' != strtoupper($SECFieldsVar)) {
		...
		foreach ($Payments as $PaymentsVar => $PaymentsVal) {
			...
			foreach ($CurrentPayment as $CurrentPaymentVar => $CurrentPaymentVal) {
				if ('ORDER_ITEMS' != strtoupper($CurrentPaymentVar)) {
	...
	public function DoExpressCheckoutPayment( $DataArray) {
		...
		foreach ($Payments as $PaymentsVar => $PaymentsVal) {
			...
			foreach ($CurrentPayment as $CurrentPaymentVar => $CurrentPaymentVal) {
				...
				} elseif ('REDEEMED_OFFERS' == strtoupper($CurrentPaymentVar)) {
				...
				} elseif ('MERCHANT_DATA_VARS' == strtoupper($CurrentPaymentVar)) {
	 ...
	 * @return \mixed[]
	 */
	public function BMCreateButton( $DataArray) {
	 ...
	 * @return \mixed[]
	 */
	public function BMUpdateButton( $DataArray) {
	 ...
	 */
	public function BMGetButtonDetails( $HostedButtonID) {
	 ...
	 * @return \mixed[]
	 */
	public function BMGetInventory( $DataArray) {
	 ...
	 * @return \mixed[]
	 */
	public function BMManageButtonStatus( $DataArray) {
	 ...
	 * @return \mixed[]
	 */
	public function BMSetInventory( $DataArray) {
