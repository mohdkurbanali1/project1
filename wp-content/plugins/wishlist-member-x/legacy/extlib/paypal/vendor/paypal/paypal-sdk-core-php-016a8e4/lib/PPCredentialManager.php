	...
	private function initCredential($config) {
		...
		while (in_array($key, $arrayPartKeys)){
			...
			if(isset($credArr[$key.".Signature"]) 
					&& null != $credArr[$key.".Signature"] && "" != $credArr[$key.".Signature"]){
				...
				if (isset($credArr[$key.'.AppId'])) {				
					...
					&& null != $credArr[$key.".CertPath"] && "" != $credArr[$key.".CertPath"]){
			...
			if ($userName && null == $this->defaultAccountName) {
