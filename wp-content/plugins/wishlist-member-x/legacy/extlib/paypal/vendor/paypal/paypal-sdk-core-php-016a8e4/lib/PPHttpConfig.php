...
class PPHttpConfig {
	...
	public function __construct($url=null, $method=self::HTTP_POST) {
		...
		// If SSL Version is NSS then we removed the "CURLOPT_SSL_CIPHER_LIST => 'TLSv1'"" part of the array as this
		// causes the purchase links of Paypal Express checkout to fail.
		$curl       = curl_version();
        $sslVersion = isset($curl['ssl_version']) ? $curl['ssl_version'] : '';
        if (0 === substr_compare($sslVersion, "NSS/", 0, strlen("NSS/"))) {
	...
	public function setHttpProxy($proxy) {
		...
		if(false == $urlParts || !array_key_exists("host", $urlParts))
