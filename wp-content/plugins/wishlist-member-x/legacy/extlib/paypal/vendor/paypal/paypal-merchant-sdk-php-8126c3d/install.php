...
define('COMPOSER_URL', 'https://raw.github.com/paypal/merchant-sdk-php/stable/composer.json');
...
{
	natsort($tagArray);
	if(0 === strpos($inputTag, '*') )
	{
		return end($tagArray);
	}
	else if((false !== (strpos($inputTag, '>'))) && (strpos($inputTag, '>') >=0))
	{
	...
	else
	{
		if(0 === strpos($inputTag, '<='))
		{
			...
			{
				if(1 == version_compare($strippedTag, strtolower($version)) || 0 == version_compare($strippedTag, strtolower($version)))
				{
					$tag = $version;
				}
			}
		...
		else if(0 === strpos($inputTag, '<'))
		{
			$strippedTag = 'v'.str_replace('<', '', $inputTag);
			foreach ($tagArray as $version)
			{
				if(1 == version_compare($strippedTag, strtolower($version)))
				{
					$tag = $version;
				}
			}
		...
		else if(strpos($inputTag, '*'))
		{
			...
			foreach ($tagArray as $version)
			{
				if(1 == strpos($version, $exp['0']) && -1 == version_compare($tag, $version))
				{
					$tag = $version;
				}
			}
...
function extractRef($url)
{
	...
	if(0 === strpos($reference['0']->ref, 'refs/tags/'))
	{
