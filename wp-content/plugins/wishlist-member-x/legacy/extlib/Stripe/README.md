...
```php
$stripe = new \WLMStripe\StripeClient('sk_test_BQokikJOvBiI2HlWgH4olfQ2');
...
$curl = new \WLMStripe\HttpClient\CurlClient();
$curl->setTimeout(10); // default is \WLMStripe\HttpClient\CurlClient::DEFAULT_TIMEOUT
$curl->setConnectTimeout(5); // default is \WLMStripe\HttpClient\CurlClient::DEFAULT_CONNECT_TIMEOUT

echo $curl->getTimeout(); // 10
echo $curl->getConnectTimeout(); // 5

// tell Stripe to use the tweaked client
\WLMStripe\ApiRequestor::setHttpClient($curl);
...
```php
// set up your tweaked Curl client
$curl = new \WLMStripe\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
\WLMStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\WLMStripe\WLM_Stripe::setLogger($logger);
```
...
```php
$curl = new \WLMStripe\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
\WLMStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\WLMStripe\WLM_Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
\WLMStripe\WLM_Stripe::setMaxNetworkRetries(2);
```
...
```php
\WLMStripe\WLM_Stripe::setEnableTelemetry(false);
```
...
```php
\WLMStripe\WLM_Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
