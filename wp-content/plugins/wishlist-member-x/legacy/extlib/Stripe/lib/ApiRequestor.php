...
class ApiRequestor {
	...
	private static function _telemetryJson( $requestTelemetry) {
		...
		WLM_Stripe::getLogger()->error('Serializing telemetry payload failed!');
	...
	private static function _defaultHeaders( $apiKey, $clientInfo = null) {
		$uaString = 'Stripe/v1 PhpBindings/' . WLM_Stripe::VERSION;

		$langVersion    = \PHP_VERSION;
		$uname_disabled = \in_array('php_uname', \explode(',', \ini_get('disable_functions')), true);
		$uname          = $uname_disabled ? '(disabled)' : \php_uname();

		$appInfo = WLM_Stripe::getAppInfo();
		$ua      = [
			'bindings_version' => WLM_Stripe::VERSION,
			'lang' => 'php',
	...
	private function _requestRaw( $method, $url, $params, $headers) {
		...
		if (!$myApiKey) {
			$myApiKey = WLM_Stripe::$apiKey;
		}

		if (!$myApiKey) {
			$msg = 'No API key provided.  (HINT: set your API key using '
			  . '"WLM_Stripe::setApiKey(<API-KEY>)".  You can generate API keys from '
		...
		if (WLM_Stripe::$apiVersion) {
			$defaultHeaders['Stripe-Version'] = WLM_Stripe::$apiVersion;
		}

		if (WLM_Stripe::$accountId) {
			$defaultHeaders['Stripe-Account'] = WLM_Stripe::$accountId;
		}

		if (WLM_Stripe::$enableTelemetry && null !== self::$requestTelemetry) {
