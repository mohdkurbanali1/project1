...
class BaseStripeClient implements StripeClientInterface {
	...
	/** @var \WLMStripe\Util\RequestOptions */
	private $defaultOpts;
	...
	public function __construct( $config = []) {
		if (\is_string($config)) {
			...
			throw new \WLMStripe\Exception\InvalidArgumentException('$config must be a string or an array');
		}
		...
		$this->defaultOpts = \WLMStripe\Util\RequestOptions::parse([
	 ...
	 * @param array|\WLMStripe\Util\RequestOptions $opts the special modifiers of the request
	 *
	 * @return \WLMStripe\StripeObject the object returned by Stripe's API
	 */
	public function request( $method, $path, $params, $opts) {
		$opts                          = $this->defaultOpts->merge($opts, true);
		$baseUrl                       = $opts->apiBase ? $opts->apiBase : $this->getApiBase();
		$requestor                     = new \WLMStripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
	...
	public function requestCollection( $method, $path, $params, $opts) {
		...
		if (!( $obj instanceof \WLMStripe\Collection )) {
			...
			$msg            = "Expected to receive `WLMStripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";

			throw new \WLMStripe\Exception\UnexpectedValueException($msg);
		}
	...
	/**
	 * @param \WLMStripe\Util\RequestOptions $opts
	 *
	 * @throws \WLMStripe\Exception\AuthenticationException
	 *
	 * @return string
	 */
	private function apiKeyForRequest( $opts) {
		...
		if (null === $apiKey) {
			...
			throw new \WLMStripe\Exception\AuthenticationException($msg);
		}
	 ...
	 *
	 * @throws \WLMStripe\Exception\InvalidArgumentException
	 */
	private function validateConfig( $config) {
		// api_key
		if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('api_key must be null or a string');
		}

		if (null !== $config['api_key'] && ( '' === $config['api_key'] )) {
			$msg = 'api_key cannot be the empty string';

			throw new \WLMStripe\Exception\InvalidArgumentException($msg);
		}
		...
		if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('client_id must be null or a string');
		}

		// stripe_account
		if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
		}

		// stripe_version
		if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
		}

		// api_base
		if (!\is_string($config['api_base'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('api_base must be a string');
		}

		// connect_base
		if (!\is_string($config['connect_base'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('connect_base must be a string');
		}

		// files_base
		if (!\is_string($config['files_base'])) {
			throw new \WLMStripe\Exception\InvalidArgumentException('files_base must be a string');
		}

		// check absence of extra keys
		$extraConfigKeys = \array_diff(\array_keys($config), \array_keys($this->getDefaultConfig()));
		if (!empty($extraConfigKeys)) {
			throw new \WLMStripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . \implode(',', $extraConfigKeys));
		}
	}
}
