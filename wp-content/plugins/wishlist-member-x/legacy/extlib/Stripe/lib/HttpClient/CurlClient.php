...
namespace WLMStripe\HttpClient;

use WLMStripe\Exception;
use WLMStripe\WLM_Stripe;
use WLMStripe\Util;
...
class CurlClient implements ClientInterface {
	...
	/** @var \WLMStripe\Util\RandomGenerator */
	 ...
	 * @param null|\WLMStripe\Util\RandomGenerator $randomGenerator
	 */
	 ...
	 *   <li>\WLMStripe\Util\CaseInsensitiveArray $rheaders The response headers</li>
	...
	public function request( $method, $absUrl, $headers, $params, $hasFile) {
		...
		if (\is_callable($this->defaultOptions)) { // call defaultOptions callback, set options to return value
			$opts = \call_user_func_array($this->defaultOptions, $args);
			if (!\is_array($opts)) {
		...
		if (( 'post' === $method ) && ( WLM_Stripe::$maxNetworkRetries > 0 )) {
		...
		$opts[\CURLOPT_CAINFO]         = WLM_Stripe::getCABundlePath();
		if (!WLM_Stripe::getVerifySslCerts()) {
			$opts[\CURLOPT_SSL_VERIFYPEER] = false;
		}

		if (!isset($opts[\CURLOPT_HTTP_VERSION]) && $this->getEnableHttp2()) {
			// For HTTPS requests, enable HTTP/2, if supported
			$opts[\CURLOPT_HTTP_VERSION] = \CURL_HTTP_VERSION_2TLS; // phpcs:ignore PHPCompatibility.Constants.NewConstants.curl_http_version_2tlsFound
		}
	 ...
	 * @param int $errno
	 * @param int $rcode
	 * @param array|\WLMStripe\Util\CaseInsensitiveArray $rheaders
	 * @param int $numRetries
	 *
	 * @return bool
	 */
	private function shouldRetry( $errno, $rcode, $rheaders, $numRetries) {
		if ($numRetries >= WLM_Stripe::getMaxNetworkRetries()) {
			return false;
		}
	...
	private function sleepTime( $numRetries, $rheaders) {
		...
		$sleepSeconds = \min(
			WLM_Stripe::getInitialNetworkRetryDelay() * 1.0 * 2 ** ( $numRetries - 1 ),
			WLM_Stripe::getMaxNetworkRetryDelay()
		);
		...
		$sleepSeconds = \max(WLM_Stripe::getInitialNetworkRetryDelay(), $sleepSeconds);

		// And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
		$retryAfter = isset($rheaders['retry-after']) ? (float) ( $rheaders['retry-after'] ) : 0.0;
		if (\floor($retryAfter) === $retryAfter && $retryAfter <= WLM_Stripe::getMaxRetryAfter()) {
