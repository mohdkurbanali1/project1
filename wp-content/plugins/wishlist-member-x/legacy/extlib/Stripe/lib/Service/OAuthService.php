...
class OAuthService extends \WLMStripe\Service\AbstractService {
	 ...
	 *
	 * @return \WLMStripe\StripeObject the object returned by Stripe's Connect API
	 */
	...
	public function authorizeUrl( $params = null, $opts = null) {
		...
		$query = \WLMStripe\Util\Util::encodeParameters($params);
	 ...
	 * @param null|array $opts
	 *
	 * @throws \WLMStripe\Exception\OAuth\OAuthErrorException if the request fails
	 *
	...
	private function _getClientId( $params = null) {
		...
		if (null === $clientId) {
			$msg = 'No client_id provided. (HINT: set your client_id using '
			  . '`new \WLMStripe\StripeClient([clientId => <CLIENT-ID>
	 ...
	 *
	 * @return \WLMStripe\Util\RequestOptions
	 */
	private function _parseOpts( $opts) {
		if (\is_array($opts)) {
			if (\array_key_exists('connect_base', $opts)) {
				...
				// \WLMStripe\Service\OAuthService from \WLMStripe\OAuth, where `connect_base`
				// was the name of the parameter that behaves as `api_base` does here.
				throw new \WLMStripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
			}
		}

		return \WLMStripe\Util\RequestOptions::parse($opts);
	}
