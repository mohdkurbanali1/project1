...
class AuthorizeNetDPM extends AuthorizeNetSIM_Form {
	...
	public static function directPostDemo( $url, $api_login_id, $transaction_key, $amount = '0.00', $md5_setting = '') {
		...
		if (!count($_POST) && !count($_GET)) {
		...
		} elseif (count($_POST)) { // Step 2: Handle AuthorizeNet Transaction Result & return snippet.
			...
			if ($response->isAuthorizeNet()) {
				...
				echo esc_url_raw(self::getRelayResponseSnippet($redirect_url));
			} else {
				echo 'Error -- not AuthorizeNet. Check your MD5 Setting.';
			}
		} elseif (!count($_POST) && count($_GET)) { // Step 3: Show receipt page to customer.
			if (1 == $_GET['response_code']) {
				echo 'Thank you for your purchase! Transaction id: ' . esc_html($_GET['transaction_id']);
			} else {
			  echo 'Sorry, an error occurred: ' . esc_html($_GET['response_reason_text']);
			}
		}
	}
