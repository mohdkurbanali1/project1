...
class AuthorizeNetCP_Response extends AuthorizeNetResponse {
	...
	public function __construct( $response, $delimiter, $encap_char, $custom_fields) {
		if ($response) {
			...
			$this->approved = ( self::APPROVED == $this->response_code );
			$this->declined = ( self::DECLINED == $this->response_code );
			$this->error    = ( self::ERROR == $this->response_code );
			$this->held     = ( self::HELD == $this->response_code );
	...
	public function isAuthorizeNet( $api_login_id = false, $md5_setting = false) {
		...
		return ( strtoupper(md5($md5_setting . $api_login_id . $this->transaction_id . $amount)) == $this->md5_hash );
	}
