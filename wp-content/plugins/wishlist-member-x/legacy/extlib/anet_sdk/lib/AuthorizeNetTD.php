...
class AuthorizeNetTD extends AuthorizeNetRequest {
	...
	public function getSettledBatchListForMonth( $month = false, $year = false) {
		$month               = ( $month ? $month : wlm_date('m') );
		$year                = ( $year ? $year : wlm_date('Y') );
		$firstSettlementDate = substr(wlm_date('c', mktime(0, 0, 0, $month, 1, $year)), 0, -6);
		$lastSettlementDate  = substr(wlm_date('c', mktime(0, 0, 0, $month+1, 0, $year)), 0, -6);
	...
	public function getTransactionsForDay( $month = false, $day = false, $year = false) {
		$transactions        = array();
		$month               = ( $month ? $month : wlm_date('m') );
		$day                 = ( $day ? $day : wlm_date('d') );
		$year                = ( $year ? $year : wlm_date('Y') );
		$firstSettlementDate = substr(wlm_date('c', mktime(0, 0, 0, (int) $month, (int) $day, (int) $year)), 0, -6);
		$lastSettlementDate  = substr(wlm_date('c', mktime(0, 0, 0, (int) $month, (int) $day, (int) $year)), 0, -6);
