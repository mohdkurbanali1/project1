 ...
 *
 * @package WishListMember/APIMethods
 */
...
if ( !class_exists( 'WLMAPIMethods' ) ) {
	...
	class WLMAPIMethods {
		...
		public function getAPIkey( $raw = false ) {
			...
			if ( !$cache ) {
				$api_key = $wpdb->get_var( 'SELECT option_value FROM ' . esc_sql( wishlistmember_instance()->table_names->options ) . " WHERE option_name = 'WLMAPIKey'" );
		...
		public function get_levels( $raw = false ) {
			...
			if ( $this->internal && function_exists( 'WishListMemberAPIRequest' ) ) {
		...
		public function get_level_terms( $level_id ) {
			...
			$results = $wpdb->get_results( $wpdb->prepare( 'SELECT content_id FROM `' . esc_sql( wishlistmember_instance()->table_names->contentlevels ) . "` WHERE type = '~CATEGORY' AND level_id = %s", $level_id ) );
		...
		public function get_member_by( $field, $value, $raw = false ) {
			//Run our request internally if available
			if ( $this->internal && function_exists( 'get_user_by' ) ) {
		...
		public function member_can_access( $user_id, $object_type, $object_id, $raw = false ) {
			...
			if ( isset($response['content'][$object_type][0]['Protected']) && !$response['content'][$object_type][0]['Protected'] ) {
				return  true;
			}
