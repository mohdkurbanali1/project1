...
if (!$this->get_option('dismiss-wlm4is-ad')) {
	printf ('
	...
	', esc_url( $adlink ), esc_html( $adtext ) );
} else {
	printf('<p style="float:right; margin:-2em 0 0 0"><a href="%s" target="_blank">%s</a></p>', esc_url( $adlink ), esc_html( $adtext ) );
}
